...
buttonParrafo.onclick = function () {
    ...
    if (contenedor) {
        // Verificando que exista el menú dentro del contenedor
        const menu = contenedor.querySelector("header");

        if (menu) {
            // Solicitando el texto del párrafo al usuario
            let texto = prompt("Agregue un párrafo a su página web");

            // Verificando que el texto no esté vacío o nulo
            if (texto && texto.trim() !== "") {
                ...
                // Agregando el párrafo como hijo del contenedor
